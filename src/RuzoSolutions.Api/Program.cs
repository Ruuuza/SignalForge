...
using RuzoSolutions.Api;
using RuzoSolutions.Application;
using RuzoSolutions.Infrastructure;

var builder = WebApplication.CreateBuilder(args);
var connectionString = builder.Configuration.GetConnectionString("RuzoSolutions") ?? "Data Source=data/ruzo-solutions.db";
...
builder.Services.AddDbContext<RuzoSolutionsDbContext>(options => options.UseSqlite(connectionString));
...
{
    await DemoData.InitializeAsync(scope.ServiceProvider.GetRequiredService<RuzoSolutionsDbContext>());
}
